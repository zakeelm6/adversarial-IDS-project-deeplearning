...
\begin{frame}{D\'efense 1 --- Adversarial Training}
\begin{columns}[T]
\column{0.54\textwidth}
\begin{block}{Principe}
Re-entra\^iner le mod\`ele sur un mix donn\'ees propres + adversariales.
\end{block}
\vspace{0.3em}
\textbf{\`A chaque epoch :}
\begin{enumerate}
    \item G\'en\'erer $X_\text{adv}$ = FGSM($X_\text{train}$, $\varepsilon$=0.1)
    \item Batch mixte : 50\% propres + 50\% adversariaux
    \item Mise \`a jour des poids
\end{enumerate}

\vspace{0.3em}
Le mod\`ele apprend que l'attaque \emph{et sa version modifi\'ee} sont toutes les deux des attaques.
$\Rightarrow$ fronti\`ere de d\'ecision plus large.

\column{0.43\textwidth}
\begin{alertblock}{Trade-off}
Entra\^inement plus difficile $\Rightarrow$ l\'eg\`ere perte sur donn\'ees propres.
\medskip

F1 propres : 0.911 $\rightarrow$ \textbf{0.897}
\end{alertblock}
\end{columns}
\end{frame}
...
\vspace{0.2em}
\begin{center}
\footnotesize Le Feature Squeezing donne le meilleur compromis robustesse / performance.
\end{center}
\end{frame}
...
\begin{itemize}
    \item MLP baseline : F1 = \textbf{0.911} sur donn\'ees propres
    \item FGSM/PGD font \'evader \textbf{27\%} des attaques ($\varepsilon=0.1$)
    \item \textbf{Feature Squeezing} : meilleure d\'efense dans ce cadre (evasion 23.4\%)
    \item L'adversarial training est limit\'e par le nombre d'epochs (10 vs 30)
\end{itemize}
\end{block}
...
\begin{itemize}
    \item Combiner les deux d\'efenses
    \item Adversarial training avec PGD (pas seulement FGSM)
    \item Randomized smoothing (d\'efense certifi\'ee)
    \item \'Evaluation sur d'autres datasets (CICIDS, KDD)
\end{itemize}
\end{exampleblock}

\vspace{0.4em}
\begin{alertblock}{Le trade-off fondamental}
Robustesse $\uparrow$ \quad $\Longleftrightarrow$ \quad Performance donn\'ees propres $\downarrow$
\end{alertblock}
\end{columns}
\end{frame}
