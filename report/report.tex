...
Deux d\'efenses sont \'etudi\'ees : l'adversarial training (deux variantes) et le feature squeezing.
Nous montrons que l'adversarial training FGSM (Approche~1) d\'egrade la robustesse (evasion~36.7\%),
que l'adversarial training PGD (Approche~2) \'echoue \'egalement (evasion~40.2\%),
et que le feature squeezing obtient le meilleur r\'esultat (evasion~\textbf{23.4\%} sous PGD, F1~=~\textbf{0.852}).
\end{abstract}
...
\subsubsection*{Approche 2 --- PGD Adversarial Training (PGD-AT)}

\textbf{Motivations :} corriger les trois d\'efauts de l'Approche~1 en utilisant PGD-7 comme
attaque d'entra\^inement (m\^eme famille que l'\'evaluation) et en entra\^inant sur 100\% d'exemples
adversariaux (\'elimination du distribution shift).

\textbf{Formulation min-max de Madry et al.~\cite{madry2018} :}
\[
\min_\theta\; \mathbb{E}_{(x,y)}\!\left[\max_{\|\delta\|_\infty \leq \varepsilon} \mathcal{L}(f_\theta(x+\delta), y)\right]
\]

\`A chaque epoch : (1) g\'en\'erer $X_{\text{adv}}$ = PGD-7($X_{\text{train}}$, $\varepsilon=0.1$, $\alpha=0.01$),
(2) mettre \`a jour les poids uniquement sur $X_{\text{adv}}$ (pas de mix).

\textbf{R\'esultat :} Evasion rate sous PGD = \alert{40.2\%} --- encore plus \'elev\'e.

\textbf{Analyse :} entra\^iner sur 100\% d'exemples PGD sans aucune donn\'ee propre provoque
un \emph{oubli catastrophique} de la distribution naturelle.
Le mod\`ele ne voit jamais les donn\'ees propres et surfit la distribution adversariale PGD-7,
sans g\'en\'eraliser aux exemples PGD-40 de l'\'evaluation (gap PGD-7 vs PGD-40 plus important encore).

\textbf{Conclusion sur l'adversarial training :} les deux variantes d\'egradent la robustesse sur ce
dataset. Ceci illustre la difficult\'e intrins\`eque de l'adversarial training sur des donn\'ees tabulaires
r\'eseau, o\`u les features sont h\'et\'erog\`enes et les perturbations ne suivent pas de structure
spatiale exploitable par le mod\`ele.
...
\textbf{Constat principal :} les deux variantes d'adversarial training (FGSM-AT et PGD-AT) produisent
...
robustness gap, distribution shift, et oubli catastrophique.
...
\begin{itemize}[leftmargin=1.5em, noitemsep]
    \item Le \textbf{feature squeezing} est la meilleure d\'efense : evasion~24.5\% sous PGD
    ...
    \item L'\textbf{adversarial training PGD} (Approche~2) \'echoue \'egalement (evasion~40.2\%)
    ...
    sur des donn\'ees tabulaires r\'eseau h\'et\'erog\`enes.
\end{itemize}
